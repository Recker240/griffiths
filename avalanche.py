...
def no_execution_avalanches_brain(P:np.ndarray, T:int, qtde:int, file_loc:str, lamb:float, override_qtde=True):
...
def streamlit_plotly_avalanche_plotter(M_0: int, alpha: float, network_p: float, b: int, s: int, mode: str, T: int, fips, qtde: int, Nhist: int, wherefit: bool, override_qtde=True, plot_cumulative=False):
    ...
    fig_crack = make_subplots(cols=2,rows=1)
    ...
    for i, iteration_p in enumerate(tqdm(iteration_ps_list, colour='green')):
        ...
        avalanche_table = no_execution_avalanches_brain(P, T, qtde, data_file_loc, lamb, override_qtde)
    ...
    return fig_prob, fig_acum, fig_crack
...
def streamlit_matplotlib_avalanche_plotter(M_0: int, alpha: float, network_p: float, b: int, s: int, mode: str, T: int, fips, qtde: int, Nhist: int, wherefit: bool, override_qtde=True, plot_cumulative=False):
    ...
    for i, iteration_p in enumerate(tqdm(iteration_ps_list, colour='green')):
        ...
        avalanche_table = no_execution_avalanches_brain(P, T, qtde, data_file_loc, lamb)
        grouped = avalanche_table.groupby("Durations")
        med_spik = grouped.mean()
        ...
        if i==len(iteration_ps_list)-1 and len(fips) > 1:
    ...
    return fig_prob, fig_cum, fig_crack
...
def avalanche_site_maker():
    ...
    with network_choosing_cols[0]:
        selected_M_0 = st.selectbox("Número de neurônios no módulo mais interno:", possible_M_0s, placeholder="M_0=2")
    ...
    with network_choosing_cols[1]:
        selected_alphap = st.selectbox(r"$\alpha$ e $p$ da rede", possible_alphap, placeholder="alpha=2_network_p=0.25")
    ...
    with network_choosing_cols[2]:
        selected_bs = st.selectbox("modularização da rede:", possible_bs, placeholder="b=2_s=12")
    ...
    with network_choosing_cols[3]:
        selected_T = st.selectbox("Passos de tempo iterados:", possible_T, placeholder="T=1000000")
        T = int(selected_T[selected_T.index("=")+1:])
    folder += f"{selected_T}/"
    possible_netslambs = sorted(os.listdir(folder))

    # files = folder + np.605array(os.listdir(folder))
    lamb_texts = []
    lamb_values = []
    ...
    with fit_selecting_cols[0]:
        ...
        ignore_fits = st.checkbox("Igonorar Ajuste de dados")
    ...
    selected_lamb_texts = st.multiselect("Selecione valores desejados para lambda. Eles são só para otimização, é possivel deselecioná-los posteriormente.", lamb_texts,[], on_change=lambda: time.sleep(2))
    ...
    if bib == "Plotly":
        fig_prob, fig_acum, fig_crack = streamlit_plotly_avalanche_plotter(M_0, alpha, network_p, b, s, 'r1', T, lamb_values, 0, 100, adj, plot_cumulative=plot_cumulative)
    elif bib == "Matplotlib":
        fig_prob, fig_acum, fig_crack = streamlit_matplotlib_avalanche_plotter(M_0, alpha, network_p, b, s, 'r1', T, lamb_values, 0, 100, adj, plot_cumulative=plot_cumulative)

    if bib == "Plotly":
        st.plotly_chart(fig_prob)
        if plot_cumulative: st.plotly_chart(fig_acum)
        st.plotly_chart(fig_crack)
    elif bib == "Matplotlib":
        st.pyplot(fig_prob,dpi=450)
        if plot_cumulative: st.pyplot(fig_acum,dpi=450)
        if len(lamb_values)>2 and adj != 'no': st.pyplot(fig_crack,dpi=450)
